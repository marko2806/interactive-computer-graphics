...
template <typename T>
Vector<T> operator*(const Vector<T>& m1, const Matrix<T>& m2) {
	if (m1.vector.size() == m2.getM()) {
		std::vector<T> resultVector;
		for (int i = 0; i < m2.getN(); i++) {
			T element = 0.0;
			for (int j = 0; j < m1.vector.size(); j++) {
				element += m2.matrix[j][i] * m1.vector[j];
			}
			resultVector.push_back(element);
		}
		return Vector<T>(resultVector);
	}
	return NULL;
}
