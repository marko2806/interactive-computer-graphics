...
Vector<double> Vector<T>::normalized() {
	...
	for (int i = 0; i < this->vector.size(); i++) {
		norm += (double)(this->vector[i]) * (double)(this->vector[i]);
	}
	norm = sqrt(norm);
	for (int i = 0; i < this->vector.size(); i++) {
		result.vector[i] = (double)this->vector[i] / norm;
	}
	return result;
}
...
double operator*(const Vector<T>& v1, const Vector<T>& v2) {
	double result = 0.0;
	int size = std::min(v1.vector.size(), v2.vector.size());
	for (int i = 0; i < size; i++) {
		result = result + ((double)v1.vector[i] * (double)v2.vector[i]);
	}
	return result;
}
template <typename T>
Vector<double> operator*(const double scalar, const Vector<T>& v) {
	Vector<double> result(v.vector.size());
	for (int i = 0; i < v.vector.size(); i++) {
		result.vector[i] =(double)v.vector[i] * scalar;
	}
	return result;
}
template <typename T>
Vector<double> operator*(const Vector<T>& v, const double scalar) {
	Vector<double> result(v.vector.size());
	for (int i = 0; i < v.vector.size(); i++) {
		result.vector[i] = (double)v.vector[i] * scalar;
	}
