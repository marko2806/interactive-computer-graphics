...
void getPolygonCoeficients(std::vector<Poligon>* polygons) {
	for (int i = 0; i < polygons->size(); i++) {
		...
		Vector<double> a1({ x2 - x1, y2 - y1, z2 - z1 });
		Vector<double> a2({ x3 - x1, y3 - y1, z3 - z1 });
		Vector<double> product = Vector<double>::crossProduct3D(&a1,&a2);
		(*polygons)[i].a = product.vector[0];
		(*polygons)[i].b = product.vector[1];
		(*polygons)[i].c = product.vector[2];
	}
}
